...
<div className="flex flex-col items-center gap-1">
  ...
  >
    Change Calendar Settings
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
        ...
        {days.map((d, i) => {
          if (!d) return <div key={i} className="bg-gray-50 rounded-2xl min-h-[100px]"></div>

          const date = viewDate.date(d)
          const fullDate = dayjs(`${viewDate.year()}-${viewDate.month() + 1}-${d}`)
          const inRange = fullDate.isSameOrAfter(today, 'day') && fullDate.isSameOrBefore(eightMonthsAhead, 'day')
          const dayEvents = eventsOnDay(d)
          const freeSlots = showFreeSpots && inRange ? findFreeSlots(dayEvents, date.format('YYYY-MM-DD'), dayStart, dayEnd) : []
          ...
          <div
            ...
            onClick={() =>
              handleFreeSlotSelect(
                { start: fullDate.startOf('day'), end: fullDate.endOf('day') },
                fullDate
              )
            }
          ...
          >
            ...
            >
              ...
              {/* Show free slots */}
              {showFreeSpots && inRange && !isFullyFree && freeSlots.map((slot, idx) => (
                <button
                  key={idx}
                      onClick={(e) => {
                      e.stopPropagation();
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-lg">
      <h3 className="text-lg font-bold mb-4">Select your preferred time range</h3>
      ...
      <div className="mb-4 flex items-center">
        <input
          ...
          className="w-4 h-4"
        />
        <label htmlFor="saveForGroup" className="mr-2 text-sm">Save for this group</label>
      </div>
      ...
      <div className="mb-4 flex items-center">
          <input
            ...
            className="w-4 h-4"
          />
          <label htmlFor="privacyToggle" className="mr-2 text-sm">Make my event titles public</label>
        </div>
